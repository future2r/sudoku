...
public final class Grid<T> {
	 ...
	 */
	public static <T> Grid<T> of(final Class<T> valueType, final int columns, final int rows) {
		return new Grid<>(valueType, columns, rows);
	}
	 ...
	 */
	public static <T> Grid<T> copyOf(final Grid<T> original) {
	...
	private Grid(final Class<T> valueType, final int columns, final int rows) {
		if (columns <= 0)
		...
		this.rows = rows;
		this.values = (T[]) Array.newInstance(valueType, this.columns * this.rows);
	}

	private Grid(final Grid<T> other) {
	 ...
	 */
	public Column<T> column(int columnIndex) {
	 ...
	 */
	public Section<T> section(int startColumn, int startRow, int columns, int rows) {
